...
public class HAgeCommand implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if(commandSender.hasPermission("equination.hage")) {
            if(commandSender instanceof Player) {
                ...
                if(entity instanceof Horse) {
                    if(strings.length > 0) {
                        ...
                        if(cHorse != null) {
                            cHorse.setAge(Integer.parseInt(strings[0]));
                            player.sendMessage(ChatColor.translateAlternateColorCodes('&',
                                    Objects.requireNonNull(EquiNation.getInstance().getConfig().getString("messages.changed-age"))
                                            .replace("%age%", strings[0])));
                    ...
                    } else player.sendMessage(ChatColor.RED + "Usage: /hage <age>");
